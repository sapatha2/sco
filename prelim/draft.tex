...
\usepackage{amsfonts}
\usepackage{titling}
\title{Development of an effective model for SrCuO$_2$ using \textit{ab-initio} quantum Monte Carlo density-matrix downfolding}
\author{Shivesh Pathak, Advisor: Lucas Wagner}
\date{TBD}
...
\begin{abstract}
\normalsize
Understanding the low energy behavior of the cuprate superconductors has been an open question in theoretical condensed matter physics for over thirty years. Various model Hamiltonians have been proposed and tested to describe different phases under variation in doping and temperature, but the development and validation of these models have not been systematic. We propose that a density matrix downfolding approach \cite{10.3389/fphy.2018.00043, doi:10.1063/1.4927664} using \textit{ab-initio} quantum Monte Carlo can generate and validate an accurate low-energy model for the electron-doped cuprate SrCuO$_2$ at zero temperature and dopings of x=0, 0.125, 0.25 in a systematic way beginning directly from the \textit{ab-initio} Hamiltonian. By conducting a similar downfolding for the isostructural non-superconducting nickelate SrNiO$_2$, and comparing to the effective model for SrCuO$_2$, we hope to develop a better understanding of the mechanisms for superconductivity present in the cuprates. 
\end{abstract}
\pagebreak
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{Figures/R2-pareto.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{Figures/R3-iao_basis.pdf}
...
\textbf{An approximate timeline for accomplishing each goal follows.} Generation of low-energy states and a complete descriptor space for the cuprate calculation will take the most time in the model fitting process as it requires multiple revisions and will require about two semesters. Studying finite size effects and implementing a similar downfolding to the nickelate material should take a sum total of one semester. Finite size corrections are time-gated only by computational hours (easier to come by than human hours) and the nickelate downfolding should be much easier given the work already done on the isostructural cuprate. Therefore we expect the proposed work to take another three semesters to complete.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{Figures/P2-Ni_Cu.pdf}
