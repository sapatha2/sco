...
At zero doping the cuprates are antiferromagnetic charge-transfer insulators [ref]. 
The theoretical challenge arises away from zero doping where the poorly understood superconducting, pseudo-gap, strange metal and "normal" metal phases appear [ref]. 
\ref{fig1} shows the phase diagram of a hole- and electron-doped cuprate materials with different structures. 
The superconducting phase is d-wave in character and cannot be described by phonon-induced pairings [ref]. 
In the pseudo-gap phase the d-wave gap from the superconducting state continues to exist, however the material is now metallic in character [ref] \textbf{check this!}. 
The strange metal phase is metallic but has a "strange" T-linear resistivity unlike typical metals which have quadratic-T resistivity at low temperatures [ref]. 
The "normal" metallic phase exhibits the same single particle behavior expected of a Fermi-Landau-like metal, but the plasmon excitations are very different exhibiting self-similiarity in dispersion unlike the well defined plasmonic modes of the Fermi liquid [ref].
...
Elastic magnetic measurements on La$_2$CuO$_{4+y}$ show the superconducting phase is accompanied by a spin-density wave emergence indicating a strong correlation between the two phases [ref]. 
...
For the undoped cuprates a nearest neighbor Heisenberg model can reproduce the magnetic dispersions measured in inelastic neutron scattering experiments accurate away from the magnetic Brillouin zone boundary, but fail to capture the small effects of anomalous dispersion near the boundary [ref]. 
%All refs for this part will be from Scalapino article "models"
The simplest model for the hole-doped cuprates was suggested by Anderson and is a 2D single-band Hubbard model which only includes bands of Cu d$_{x^2-y^2}$ character, which will be hybridized with the oxygen bands to some degree [ref]. 
This model is capable of producing the AFM phase at zero doping as in the strong coupling limit this model reduces to a nearest neighbor AFM Heisenberg model [ref]. 
The model even exhibits a pseudo-gap like phase in the underdoped region [ref]. Further, in the limit of $U/t \rightarrow 0$ the Hubbard model has a transition to a $d_{x^2-y^2}$ superconducting-like phase [ref]. 
It has not been shown numerically that the superconducting order parameter is non-zero as the pair spacing is taken to be very large, however [ref]. 
A similar model is the t-J model [ref, Elbio Dagatto] which is built on the idea that under doping there are itinerant holes which hop around the oxygen bands alongside the fixed S=1/2 moments on the copper sites which interact through a superexchange term with parameter J. 
The t-J model can be seen as an effective theory of the single band t-U model. 
This model exhibits superconductivity proper at J/t $\sim$ 3 with a density of $\langle n \rangle$ = 1/2 [ref], meaning that the pairing function does not vanish in the large spacing limit. 
...
This leads to a 2D 3-band Hubbard model which includes the Cu 3$d_{x^2-y^2}$ and O p bands [ref, Elbio Dagatto] and exhibits qualitatively similar behavior to the 1-band model. 
...
\textit{ab initio} quantum Monte Carlo calculations (QMC) can generate an effective model which can accurately describe low-energy properties and energetics for the infinite layer electron-doped cuprates at zero temperature and dopings of x=0, 0.125, 0.25. 
...
These states will be sampled using fixed-node diffusion Monte Carlo, a QMC algorithm which can project out low-energy states starting with appropriate trial wave functions.
...
Fixed-node error can be reduced by using optimized many-body trial wave functions of the (multi-)Slater-Jastrow form seen in [ref Umrigar 2007].
