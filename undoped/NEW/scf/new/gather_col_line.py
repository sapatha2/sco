...
def get_U_col(mo_occ1,mo_occ2,w,M0,M1):
  M=np.array([M0,M1])
  mo_dm1=np.einsum('si,ij->sij',mo_occ1,np.eye(mo_occ1.shape[1],mo_occ1.shape[1]))
  mo_dm2=np.einsum('si,ij->sij',mo_occ2,np.eye(mo_occ2.shape[1],mo_occ2.shape[1]))
  mo_dm=np.array([mo_dm1,mo_dm2])
...
def gather_line_col(rem,add,gsws):
  #Preliminary load ins
  a=np.load('iao.pickle')
  col_mocoeff=np.load('col_mocoeff.pickle')[:,0,:,:]
  col_moenergy=np.load('col_moenergy.pickle')[:,0,:]
  s=np.load('s.pickle')
  M0=reduce(np.dot,(a.T, s, col_mocoeff[0])) 
  M1=reduce(np.dot,(a.T, s, col_mocoeff[1])) 

  df=None
  for gsw in gsws:
    #Get MO RDM
    w=np.array([np.sqrt(gsw),np.sqrt(1-gsw)])
    mo_occ1=np.zeros((col_mocoeff.shape[:-1]))
    mo_occ1[:,:66]=1
    mo_occ2=np.zeros((col_mocoeff.shape[:-1]))
    mo_occ2[:,:66]=1
    mo_occ2[1,rem]=0
    mo_occ2[1,add]=1
    dl=mo_rdm_col(mo_occ1,mo_occ2,w)
    ...
    sigU=get_U_col(mo_occ1,mo_occ2,w,M0,M1)

    #Get energy from eigenvalues 
    e=w[1]**2*(col_moenergy[1,add]-col_moenergy[1,rem])
...
if __name__=='__main__':
  ...
  for rem,add in zip(rem_list,add_list):
    print(rem,add)
    df=gather_line_col(rem,add,gsws)
    df['add']=add
    df['rem']=rem
  ...
  print(full_df)
  print(full_df.shape)
  full_df.to_pickle('col_line_gosling.pickle')
